...
exports.imagereduceupload = async (req, res) => {
  try {
    ...
    // Upload the file to Cloudinary with reduced quality
    const response = await uploadFileToCloudinary(file, "Babbar", 30);
    ...
    res.json({
      ...
      datas:filesave // Including the response from Cloudinary
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({
